...
async function getUserById1(userId) {
  const query = `
    ...
    WHERE user_id != ? ORDER BY user_id DESC;  
  `;
...
async function getUserTransactionsWithParent(userId) {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM transactions WHERE receiver_id=? AND sender_id IN (SELECT sender_id FROM transactions WHERE receiver_id=?) order by user_id desc",
